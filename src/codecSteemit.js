...
sjcl.codec.scoopmedia = {
  ...
  keysFromPassword: function(account, password) {
    ...
    for (var i = 0; i < sjcl.codec.scoopmedia.ROLES.length; i++) {
      var role = sjcl.codec.scoopmedia.ROLES[i];
  ...
  signRecoverably: function(sec, hash, paranoia, fixedKForTesting) {
     ...
     * its X coordinate).
     *
     * note that in the Scoop blockchain, all public keys are canonically compressed and therefore the
  ...
  recoverPublicKey: function(hash, sig) {
    ...
    for (var j = 0; j <= 1; j++) {
      ...
      var y = sjcl.codec.scoopmedia._yFromX(x, hasOddParity);
  ...
  serializePublicKey: function(key, net) {
    net = net || sjcl.codec.scoopmedia.MAINNET;
    ...
    return (
      ...
      sjcl.codec.base58Check.fromBits(
        header,
        point.x,
        sjcl.codec.scoopmedia.keyChecksum
      )
    );
  },
  ...
  deserializePublicKey: function(pubKey, net) {
    ...
    var y = sjcl.codec.scoopmedia._yFromX(x, isOdd);
  ...
  deserializePrivateKey: function(wif, header) {
    header = header || sjcl.codec.scoopmedia.MAINNET.privHeader;
    var curve = sjcl.ecc.curves.k256;
  ...
  _yFromX: function(x, shouldBeOdd) {
    var CURVE = sjcl.ecc.curves.k256;
    var PIDENT = sjcl.codec.scoopmedia._getPident();
  ...
  _getPident: function() {
    if (!sjcl.codec.scoopmedia.PIDENT) {
      sjcl.codec.scoopmedia.PIDENT = sjcl.ecc.curves.k256.field.modulus
        .add(1)
        .normalize()
        .halveM()
        .halveM()
    ...
    return sjcl.codec.scoopmedia.PIDENT;
  }
};
